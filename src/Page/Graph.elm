...
init { number, windowWidth, windowHeight } =
    let
        ( board, seed ) =
            Page.GameBoard.initGameBoard defaultBoardSize (Random.initialSeed number)
...
type Msg
    ...
    | BoardSizeDecreased
    | BoardSizeIncreased
    | TileRotated Page.GameBoard.Coords
    | UndoMove
...
handleNewSeedReceived int model =
    let
        ( newBoard, newSeed ) =
            Page.GameBoard.initGameBoard model.boardSize (Random.initialSeed int)
    in
    ( { model | board = newBoard, history = [], seed = newSeed, solved = False }, Cmd.none )


handleTileRotated : Page.GameBoard.Coords -> Model -> ( Model, Cmd Msg )
handleTileRotated coords ({ board, solved, history } as model) =
    let
        newBoard =
            Page.GameBoard.rotateCell coords board
...
handleBoardSizeChange newBoardSize model =
    let
        ( newBoard, newSeed ) =
            Page.GameBoard.initGameBoard newBoardSize model.seed
    in
...
viewGameWindow boardSize tiles =
    ...
    column
        [ centerX
        , padding 40
        , Border.width Page.TileSvg.borderWidth
...
viewCell : Int -> Page.GameBoard.Coords -> Page.GameBoard.Cell -> Element Msg
viewCell tileSize coords cell =
    el
        ...
        (case cell.shape of
            Knob ->
                Page.TileSvg.knobSvg tileSize

            Bar ->
                Page.TileSvg.barSvg tileSize

            Elbow ->
                Page.TileSvg.elbowSvg tileSize

            Tee ->
                Page.TileSvg.teeSvg tileSize
